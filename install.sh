...
root=""
# 脚本存放地址 
#黑版 mico_path="${root}/root/mico.sh"
mico_path="${root}/data/mico.sh"
# 脚本开机启动
#黑版 mico_initpath="${root}/etc/init.d/mico_enable"
...
echo "请输入NodeRed服务地址,默认值[http://nodered:1880]:"
read -p "" nodered_url
...
# 下载远程脚本并检查是否成功
now=`date +%s`
mico=`curl --insecure -s –connect-timeout 4 -m 4 "https://raw.githubusercontent.com/tvgh/mico/master/mico.sh?${now}"`
# mico=`cat ./mico.sh`
